# Problem Set 2, Hangman extended edition.py
...
def match_with_gaps(my_word, other_word, letters_guessed, trigger):
    '''
    trigger: string, the letter to switch the versions of the game.
    letters_guessed: list of used letters
...
def show_possible_matches(my_word, letters_guessed, trigger):
    '''
    trigger: string, the letter to switch the versions of the game.
    my_word: string with _ characters, current guess of secret word
    ...
    for word in wordlist:
        if match_with_gaps(my_word, word, letters_guessed, trigger):
            matches.append(word)
    if len(matches) == 0:
...
def hangman_with_hints(secret_word, trigger):
    '''
    secret_word: string, the secret word to guess.
    trigger: string, the letter to switch the versions of the game.
        ...
        if letter == '*':
            if trigger == 'x' or trigger == 'k':
                print(f"Possible word matches are: "
                      f"{show_possible_matches(get_guessed_word(secret_word, letters_guessed).replace(' ', ''), letters_guessed, trigger)}")
            elif trigger == 'h' or trigger == 'o':
                print(f"Possible word matches are: "
                      f"{show_possible_matches(get_guessed_word(secret_word, letters_guessed).replace(' ', ''), letters_guessed, trigger)}")
...
if __name__ == "__main__":
    print('\n- To play without hints, press "r"'
          '\n- To play with hints, press "h"'
          '\n- To play with extended hints, press "x"'
          '\n- To play with your word and without hints (tester mode), press "w"'
          '\n- To play with your word and hint (tester mode), press "o"'
          '\n- To play with your word and extended hints (tester mode), press "k"'
          '\n- To exit game press any another key')
    trigger = input('Change mode: ').lower()
    if trigger == 'r' or trigger == 'w':
        if trigger == 'w':
            secret_word = input('Input word: ')
            hangman(secret_word)
        else:
            secret_word = choose_word(wordlist)
            hangman(secret_word)
    elif trigger == 'h' or trigger == 'x' or trigger == 'o' or trigger == 'k':
        if trigger == 'h':
            secret_word = choose_word(wordlist)
            hangman_with_hints(secret_word, trigger)
        elif trigger == 'x':
            secret_word = choose_word(wordlist)
            hangman_with_hints(secret_word, trigger)
        elif trigger == 'o':
